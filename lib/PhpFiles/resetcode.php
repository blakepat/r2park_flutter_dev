<?php
    ...
    $select = $db->query("SELECT * FROM users WHERE user_id = '".$user_id."' AND email = '".$email."' ");
    ...
    if ($count != 0) {
        // $update = $db->query("UPDATE users SET password = '".$newPass."' WHERE user_id = '".$user_id."' AND email = '".$email."' ");
        // $update = $db->query("UPDATE users SET password = '".$newPass."' WHERE email = '".$email."' ");
        
        echo json_encode("$resetCode");   
    } else {
